...
-- 4) 사용될 데이터를 중복을 최소화하여 통합하고 쉽게 접근하여 처리할 수 있도록 저장장치에 
--     저장하여 항상 사용할 수 있도록 운영하는 운영 데이터
-- 5) 실시간 접근성, 계속되는 변화, 동시 공유, 내용에 의한 참조가 특징
-- 6) 관계형 데이터베이스 : 단순한 표를 이용하여 데이터의 상호관계를 정의한 DB구조

-- # 데이터베이스의 특징
-- 1) 질의에 대하여 즉각적인 처리와 응답이 이루어짐
-- 2) Create, Update, Delete를 통하여 최신의 데이터를 유지
-- 3) 사용자들에게 원하는 데이터를 동시에 공유 가능
-- 4) 사용자가 원하는 데이터를 주소가 아닌 내용에 따라 참조 가능하다.
-- 5) 응용프로그램과 데이터베이스는 독립, 데이터의 논리적 구조와 응용프로그램은 별개로 동작한다.
